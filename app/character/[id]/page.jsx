  ...
  if (!character) {
    return (
      <div className="flex items-center justify-center h-screen">
        <p className="text-2xl font-bold text-red-600">Character not found or failed to fetch data.</p>
      </div>
    )
  }
      ...
      <Card className="bg-gray-900 text-white border-red-600 border-2">
        <CardHeader className="flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="text-center md:text-left">
            <CardTitle className="text-4xl font-extrabold text-red-600 mb-2">{character.name}</CardTitle>
            ...
            {character.comics.items.length > 0 ? (
              <ul className="space-y-2">
                {character.comics.items.map((comic, index) => (
                  <li key={index}>
                    <p className="text-lg font-semibold">{comic.name}</p>
                    <Separator className="my-2 bg-red-600/30" />
                  </li>
                ))}
              </ul>
            ) : (
              <p className="text-gray-400">No comics available for this character.</p>
            )}
          </ScrollArea>
        </CardContent>
      </Card>
